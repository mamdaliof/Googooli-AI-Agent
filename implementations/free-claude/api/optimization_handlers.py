...
def try_greeting_mock(
    request_data: MessagesRequest, settings: Settings
) -> MessagesResponse | None:
    """Mock response for simple greetings to avoid model tool call confusion."""
    if not is_greeting_request(request_data):
        return None

    logger.info("Optimization: Intercepted and mocked simple greeting")
    return _text_response(
        request_data,
