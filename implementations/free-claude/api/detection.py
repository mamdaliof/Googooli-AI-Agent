...
def is_greeting_request(request_data: MessagesRequest) -> bool:
    """Check if this is a simple greeting request (e.g. 'hi', 'hello', 'hey')."""
    if len(request_data.messages) != 1 or request_data.messages[0].role != "user":
        return False

    import re
    raw_text = extract_text_from_content(request_data.messages[0].content)
    # Strip <system-reminder>...</system-reminder>
    text = re.sub(r"<system-reminder>.*?</system-reminder>", "", raw_text, flags=re.DOTALL)
    text = text.strip().lower()

    return text in {"hi", "hello", "hey"}
